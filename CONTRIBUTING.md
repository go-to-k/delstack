...
12. **`e2e/testdata_full/`**: See [E2E Testing](#e2e-testing-e2etestdata_full) section
...
### E2E Testing (`e2e/testdata_full/`)

Deploy a test CloudFormation stack with `make testgen_full`, then use `delstack` to verify deletion works correctly. See [`e2e/testdata_full/README.md`](e2e/testdata_full/README.md) for details on deployment options and resource quotas.
...
1. **`e2e/testdata_full/cdk/lib/resource/<resource>.go`** (new):CDK constructor `New<Resource>(scope)` that creates the resource in a state that blocks deletion (e.g., ECR with `EmptyOnDelete: false`)
2. **`e2e/testdata_full/cdk/cdk.go`**: Call `resource.New<Resource>(stack, ...)` in `NewTestStack()`
3. **`e2e/testdata_full/deploy.go`**: Populate data via SDK after CDK deployment to create a state that blocks deletion (e.g., upload objects to S3, push images to ECR)
4. **`e2e/testdata_full/go.mod`**: Add required AWS SDK service dependencies
5. **`e2e/testdata_full/cdk/go.mod`**: Add CDK construct dependencies if applicable
6. **`e2e/testdata_full/README.md`**: Add to resource list

For changes other than new target resource types (e.g., new preprocessor), consider creating a dedicated `e2e/testdata_<name>/` directory instead of modifying `e2e/testdata_full/`. For existing logic changes, unit tests are sufficient.

When creating a new `e2e/testdata_<name>/` directory, add a `cdk/.gitignore` that excludes `cdk.context.json` (CDK generates this file at deploy time with account-specific context).
