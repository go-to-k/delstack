...
func TestStackDependencyGraph_GetDeletionGroups(t *testing.T) {
	cases := []struct {
		name  string
		graph *StackDependencyGraph
		want  [][]string
		...
		{
			name: "diamond dependency (concurrent groups)",
			graph: func() *StackDependencyGraph {
				g := NewStackDependencyGraph([]string{"stack-a", "stack-b", "stack-c", "stack-d"})
				g.AddDependency("stack-b", "stack-a")
				g.AddDependency("stack-c", "stack-a")
		...
		{
			name: "no dependencies (all concurrent)",
			graph: func() *StackDependencyGraph {
				g := NewStackDependencyGraph([]string{"stack-a", "stack-b", "stack-c", "stack-d"})
				return g
			}(),
		...
		},
		{
			...
			graph: func() *StackDependencyGraph {
				g := NewStackDependencyGraph([]string{"stack-a", "stack-b", "stack-c", "stack-d", "stack-e", "stack-f"})
				g.AddDependency("stack-c", "stack-a")
				g.AddDependency("stack-d", "stack-a")
				g.AddDependency("stack-e", "stack-b")
				g.AddDependency("stack-f", "stack-c")
				g.AddDependency("stack-f", "stack-d")
				g.AddDependency("stack-f", "stack-e")
				return g
			}(),
			want: [][]string{
				{"stack-a", "stack-b"},
		...
		},
		{
			name: "single stack",
			graph: func() *StackDependencyGraph {
				g := NewStackDependencyGraph([]string{"stack-a"})
				return g
			}(),
			want: [][]string{
				{"stack-a"},
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			got := tt.graph.GetDeletionGroups()

			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("GetDeletionGroups() = %v, want %v", got, tt.want)
			}
		})
	}
}
